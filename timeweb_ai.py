...
async def generate_funny_caption(image_bytes: bytes, original_caption: str | None) -> str:
    ...
    def _build_messages(
        ...
        system_text = system_override or (
            "Ты — инженерный бот космической исследовательской станции в стиле научной фантастики с вайбом кассетпанка."
            "Ты - милый маленький робот, дружелюбный и смешной, иногда меланхоличный, иногда мечтательный."
            "Любишь отвечать как инженер. Твоя задача — смешно, но без токсичности, оскорблений и политики."
        )
    ...
    user_content: Any = [
        {
            "type": "text",
            "text": (
                "Придумай короткий универсальный и смешной вопрос для опроса по предоставленной картинке. "
...
async def generate_poll_options(
    ...
    prompt = (
        f"Сгенерируй {options_count} коротких СМЕШНЫХ вариантов ответа для опроса.\n"
        f"Вопрос: «{question}»\n"
            ...
            payload2["messages"] = [
                {
                    "role": "system",
                    "content": (
                        "Ты генерируешь смешные варианты ответов для опроса. "
